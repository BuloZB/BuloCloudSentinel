<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_sentinel_web_version ?? '0.0.0', WEBUI_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'Open WebUI version (v{{sentinel_web_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						sentinel_web_VERSION: WEBUI_VERSION,
						REQUIRED_VERSION: manifest?.required_sentinel_web_version ?? '0.0.0'
					}
				)
			);
			return;
	...
	onMount(() => {
		window.addEventListener('message', async (event) => {
			if (
				!['https://SentinelWeb.com', 'https://www.SentinelWeb.com', 'http://localhost:9999'].includes(
					event.origin
				)
			)
				return;
