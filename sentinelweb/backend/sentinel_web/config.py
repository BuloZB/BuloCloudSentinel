...
from sentinel_web.internal.db import Base, get_db
from sentinel_web.utils.redis import get_redis_connection
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(sentinel_web_DIR / "alembic.ini")

        # Set the script location dynamically
        migrations_path = sentinel_web_DIR / "migrations"
...
class AppConfig:
    ...
    def __setattr__(self, key, value):
        if isinstance(value, PersistentConfig):
            ...
            if self._redis:
                redis_key = f"sentinel-web:config:{key}"
...
STATIC_DIR = Path(os.getenv("STATIC_DIR", sentinel_web_DIR / "static")).resolve()
...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.SentinelWeb.com/api/v1/custom/{CUSTOM_NAME}")
        data = r.json()
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = (
                    f"https://api.SentinelWeb.com{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.SentinelWeb.com{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        ...
        OLLAMA_BASE_URL = "http://ollama-service.sentinel-web.svc.cluster.local:11434"
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get(
    "ELASTICSEARCH_INDEX_PREFIX", "sentinel_web_collections"
)
# Pgvector
