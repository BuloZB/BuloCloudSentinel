...
from sentinel_web.env import ENABLE_FORWARD_USER_INFO_HEADERS, SRC_LOG_LEVELS
from sentinel_web.routers.files import upload_file
from sentinel_web.utils.auth import get_admin_user, get_verified_user
from sentinel_web.utils.images.comfyui import (
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-SentinelWeb-User-Name"] = user.name
                headers["X-SentinelWeb-User-Id"] = user.id
                headers["X-SentinelWeb-User-Email"] = user.email
                headers["X-SentinelWeb-User-Role"] = user.role
