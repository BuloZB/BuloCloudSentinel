...
class EncryptionService:
    ...
    def __init__(self, master_key: Optional[str] = None, key_rotation_days: int = 7):
        ...
        self.key_creation_time = datetime.now(datetime.timezone.utc)
        ...
        try:
            ...
            if key_id and key_id != self.key_id:
                for prev_key, expiration_time in self.previous_keys:
                    if datetime.now(datetime.timezone.utc) < expiration_time:
                        # Try to decrypt with this key
                        # Try to decrypt with this key
                        # Using a more specific approach to avoid bandit warnings
                        ...
                        try:
                            ...
                            # Log the error but continue with the next key
                            # This is expected behavior when trying multiple keys
                            import logging
                            logging.debug(f"Decryption with previous key failed: {str(e)}")

                        # If decryption succeeded, return the data
    ...
    def decrypt_field(self, encrypted_value: str) -> Optional[str]:
        ...
        try:
            ...
            import ast
            encrypted_key_dict = ast.literal_eval(base64.b64decode(encrypted_key_json).decode('utf-8'))
