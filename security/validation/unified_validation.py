...
SQL_INJECTION_REGEX = re.compile(r'(?i)(SELECT|INSERT|UPDATE|DELETE|DROP|ALTER|UNION|INTO|EXEC|EXECUTE|OR\s+1\s*=\s*1|\'.*--|\";|\/\*|\*\/|\b(OR|AND)\b\s+\d+\s*=\s*\d+|\b(OR|AND)\b\s+\'\w+\'\s*=\s*\'\w+\')')
...
def check_sql_injection(input_str: str) -> bool:
    ...
    single_quotes = input_str.count("'")
    double_quotes = input_str.count('"')
    ...
    # Check for LIKE attack patterns
    if "%" in input_str and "LIKE" in input_str.upper():
        return True
